...
<body>
    <div class="container">
        ...
        <main  style="margin-left:300px; padding-top: 1px;">
        <header>
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while($row2 = $result->fetch_assoc()) {
                            //get document history
                                    ...
                                    echo "<td>" . $row2["Date Published"] . "</td>";
                                    /*echo "<td>";
                                    if (!empty($row2["file_path"])) {
                                        echo "<a href='" . $row["file_path"] . "' target=_blank class='done-btn'>Download PDF</a>";
                                    }*/
                                    echo "</td>";                            
                                    echo "<td>" . $row2["d_status"] . "</td>";
                                    echo "<td>" . $row2["d_status"] . "</td>";
                                    $usql = "SELECT * FROM users WHERE ID = " . $row2["user_id"];
                                    //echo $sql;
                                    $uresult = $conn->query($usql);
                                    $urow = $uresult->fetch_assoc();
                                    $user = $urow['Username'];
                                    echo "<td>" . $user . "</td>";
                                    echo "<td>" . $row2["date_updated"] . "</td>";
                                    echo "</tr>";
                                }
                    } 
