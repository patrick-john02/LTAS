...
<section class="documents-management">
    ...
    <div class="document-list">
        <center><h3>ALL APPROVED ORDINANCES</h3></center>
        <table id="doc-table">
            <thead>
                <tr>
                    <th>Resolution No</th>
            ...
            <tbody>
                <?php
                $uid = $_SESSION['userid'];
                $sql = "SELECT * FROM documents WHERE Category = 'Ordinances' AND d_status = 'Approved' AND user_id = $uid";
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        // echo "<td><a href='Userarchive_document.php?id=$did' class='archive-btn'>Archive</a></td>"; // Add Archive Button
                        echo "<td><a href='#' class='archive-btn' data-id='$did'><i class='fas fa-archive'></i></a></td>";
                        echo "</tr>";
                    }
                } 
...
<div class="modal fade" id="archiveModal" tabindex="-1" aria-labelledby="archiveModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" id="confirmArchive">Yes</button>
            </div>
        </div>
    </div>
</div>
    <script>
        ...
        $(document).ready(function () {
    let documentIdToArchive;

    // Trigger the modal when archive button is clicked
    $('.archive-btn').on('click', function (e) {
        e.preventDefault();
        documentIdToArchive = $(this).data('id'); // Get the document ID
        $('#archiveModal').modal('show'); // Show the modal
    });

    // Handle the confirmation
    $('#confirmArchive').on('click', function () {
        if (documentIdToArchive) {
            // Make an AJAX request to archive the document
            $.ajax({
                url: 'Userarchive_document.php', // Your PHP script to handle archiving
                type: 'POST',
                data: { id: documentIdToArchive },
        ...
        $('#archiveModal').modal('hide'); // Hide the modal
    });
});
